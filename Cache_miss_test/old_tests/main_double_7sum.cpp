...
void cache_test(double* &A, int num_sum) {	
	...
	int* tmpi = new int[7*num_sum];
	for (int i = 0; i < 7*num_sum; ++i)
		tmpi[i] = f(gen);
	...
	for(int i = 0; i < num_sum; i++) {
		sum += A[tmpi[7*i]] + A[tmpi[7*i + 1]] + A[tmpi[7*i + 2]] + A[tmpi[7*i + 3]] + A[tmpi[7*i + 4]] + A[tmpi[7*i + 5]] + A[tmpi[7*i + 6]];
	...
	ofstream REZ("REZ_double_7sum.csv", ios::in|ios::app);
	REZ << "double; "  << sum << ';' << datasize << ';' << 7*num_sum << ';' << time/(1000) << ';'  << values[0] << ';' << values[1] << ';' <<  values[2] << ';' << (long double)values[2]/(time/1000) << endl ;
	REZ.close();
}
