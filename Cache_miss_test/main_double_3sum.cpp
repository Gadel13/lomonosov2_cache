...
void cache_test(double* &A, int num_sum) {	
	...
	ofstream REZ("111REZ_double_3sum.csv", ios::in|ios::app);
	REZ << "double; "  << sum << ';' << datasize << ';' << 3*num_sum << ';' << time/(1000) << ';'  << values[0] << ';' << values[1] << ';' <<  values[2] << ';' << (long double)values[2]/(time/1000) << endl ;
	REZ.close();
}
