...
void cache_test(double* &A, int num_sum) {	
	...
	int* tmpi = new int[2*num_sum];
	for (int i = 0; i < 2*num_sum; ++i)
		tmpi[i] = f(gen);
	...
	ofstream REZ("11REZ_double_2sum.csv", ios::in|ios::app);
	REZ << "double; " << sum << ';' << datasize << ';' << 2*num_sum << ';' << time/(1000) << ';'  << values[0] << ';' << values[1] << ';' <<  values[2] << ';' << (long double)values[2]/(time/1000) << endl ;
	REZ.close();
}
